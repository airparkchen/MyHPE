...
class Model(nn.Module):
    ...
    def forward(self, inputs, targets=None, meta_info=None, mode=None):
        ...
        if mode == 'train':
            ...
            # test output
            out = {}
            out['img'] = inputs['img']           #原始輸入圖像
            out['rel_trans'] = rel_trans         #相對轉移，兩隻手之間的相對位置或運動
            out['rjoint_img'] = rjoint_img       #右手關節在圖像空間中的位置
            out['joint_img'] = joint_img         #右手和左手關節在圖像空間中的位置
            out['lmano_mesh_cam'] = lmesh_cam    
            out['rmano_mesh_cam'] = rmesh_cam    #左手和右手的3D網格在相機坐標系中的位置
            out['lmano_root_cam'] = lroot_cam    
            out['rmano_root_cam'] = rroot_cam    #左手和右手的根部關節在相機坐標系中的位置
            out['lmano_joint_cam'] = ljoint_cam  
            out['rmano_joint_cam'] = rjoint_cam  #左手和右手的所有關節在相機坐標系中的位置
            out['lmano_root_pose'] = lroot_pose
            out['rmano_root_pose'] = rroot_pose  #左手和右手的根部關節的姿勢
            out['lmano_hand_pose'] = lhand_pose
            out['rmano_hand_pose'] = rhand_pose  #左手和右手的姿勢參數
            out['lmano_shape'] = lshape 
            out['rmano_shape'] = rshape          #左手和右手的形狀參數
            out['lmano_joint'] = ljoint_proj    
            out['rmano_joint'] = rjoint_proj     #左手和右手關節的投影到2D圖像空間中的位置
