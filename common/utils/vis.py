...
def vis_keypoints_with_skeleton(img, kps, kps_lines, kp_thresh=0.4, alpha=1):  #img: [H, W, 3]    kps: [3, num_kps]  kps_lines 表示哪些關節點之間應該被連線來形成骨架的列表。
...
def vis_mesh(img, mesh_vertex, alpha=0.5):
    ...
    for i in range(len(mesh_vertex)):
        p = (mesh_vertex[i][0].astype(np.int32)), mesh_vertex[i][1].astype(np.int32)    #左手的話要加上x座標尾
        print(p)
...
def save_obj(v, f, file_name='output.obj'):  #透過rmano_mesh、lmano_mesh 和mano權重輸出obj
...
def visualize_keypoints(img, kps):
    # Make a copy of the image to draw on
    img_with_kps = img.copy()

    # Define the color and radius of the circle to draw
    ...
    # Draw each keypoint
    for x, y in kps:
        x = x*68.5  #ori/256/8  ori_img/input/heatmap output
        y = y*68.5
        center = (int(x), int(y))
        print(f"{center}")
        cv2.circle(img_with_kps, center, radius, color, thickness)
